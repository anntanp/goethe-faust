#!/usr/bin/env python3
"""
Purpose: For each ProvidedCHO.creator, check whether a matching edm.RDF.Agent exists
         whose 'about' URI is a DDB organization or GND URI.
         Matching is attempted two ways: (1) URI: creator.resource == agent.about,
         (2) Label: creator.$ in agent.prefLabel[].$.
Usage:   python3 scripts/creator_agent_coverage.py data/items-excerpt-1000.json
Inputs:  DDB items JSON array
Outputs: stdout summary + data/ddbedm/creator_agent_coverage.csv
...
def agent_index(rdf: dict):
    ...
    Labels are indexed under both original and flipped forms.
    """
    by_uri   = {}
...
def check_record(rdf: dict) -> list[dict]:
    """Return one row per creator value with match details."""
    ...
    for creator in coerce_list(cho.get('creator')):
        if not isinstance(creator, dict):
            creator_str = str(creator) if creator else ''
            creator = {'$': creator_str}

        c_label    = (creator.get('$') or '').strip()
        c_resource = (creator.get('resource') or '').strip()
        ...
        rows.append({
            'creator_label':        c_label,
            'creator_resource':     c_resource,
...
def main(json_path: str):
    ...
    total          = len(all_rows)
    has_resource   = sum(1 for r in all_rows if r['creator_resource'])
    ...
    out_csv = src.parent.parent / 'data' / 'analysis' / 'creator_agent_coverage.csv'
